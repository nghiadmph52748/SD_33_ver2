<template>
  <div class="page">
    <div class="page__header">
      ...
      <div class="title-row">
        <div>
          <h1 class="title">{{ $t('store.menTitle') }} <span class="count" v-if="!loading">[{{ products.length }}]</span></h1>
          <p class="subtitle">
            {{ $t('store.menSubtitle') }}
          </p>
        </div>
    ...
    <div v-if="!error">
      <AppStoreGrid :data="products" :loading="loading" source-page="men" />
...
// Get unique brands from men products
...
// Calculate max price from men products (before filtering)
const maxProductPrice = computed(() => {
  if (!allProducts.value || !Array.isArray(allProducts.value)) {
    return 50000000;
  }
  const menProducts = allProducts.value.filter(p => p.gender === "Male");
  if (menProducts.length === 0) return 50000000;
  return Math.max(...menProducts.map(p => p.priceMax || p.price));
});
...
const products = computed(() => {
  ...
  // Start with all products filtered to Male gender
  let filtered = allProducts.value.filter(p => p.gender === "Male");
  
  // If no Male products, show some products as fallback
  if (filtered.length === 0) {
    filtered = allProducts.value.slice(0, 10);
  }

  // Filter by brands and gender (categories contains both)
