      ...
      <Transition name="fade-quick" mode="out-in">
        <article :key="status.variant" class="status-card" :class="`status--${status.variant}`">
          <header class="status-header">
            ...
            <div>
              <p class="eyebrow">{{ $t('payment.titleMomo') }}</p>
...
const amountRaw = computed(() => Number(route.query.amount ?? 0))
...
const status = computed<StatusConfig>(() => {
  // Handle MoMo orders (resultCode 0 = success)
...
// Clear cart only when payment is confirmed success (resultCode === '0')
onMounted(() => {
  if (resultCode.value === '0') {
    cartStore.clearCart()
  }
...
const detailItems = computed(() => {
  ...
  rows.push(
    ...
    { label: 'payment.details.orderId', value: orderId.value || '—' },
    { label: 'payment.details.amount', value: displayAmount.value },
    { label: 'payment.details.message', value: message.value || '—' }
  )
...
.status-card.status--success::before {
  ...
  inset: -40px;
  background-image:
    linear-gradient(120deg, rgba(193, 46, 127, 0.2) 0%, rgba(193, 46, 127, 0) 60%),
    linear-gradient(300deg, rgba(163, 26, 94, 0.2) 0%, rgba(163, 26, 94, 0) 60%);
  opacity: .6;
