...
const formRules = {
  ...
  anhNhanVien: [
    { required: true, message: 'Vui lòng chọn ảnh nhân viên' }
  ]
}
...
const handleSubmit = async () => {
  ...
  if (err.response?.data?.message?.includes('tài khoản đã tồn tại')) {
    alert('❌ Email này đã được dùng để tạo tài khoản. Vui lòng dùng email khác.')
  } else {
...
const beforeUpload = (file: File) => {
  ...
  if (!isJpgOrPng) {
    alert('Chỉ hỗ trợ upload file JPG/PNG!')
    return false
  }
