...
public class BanHangService {
    ...
    // Tính giá bán của biến thể sau khi áp dụng khuyến mãi đang hiệu lực
    private BigDecimal calculateGiaBanSauGiam(ChiTietSanPham chiTietSanPham) {
        if (chiTietSanPham == null) {
            return BigDecimal.ZERO.setScale(2, java.math.RoundingMode.HALF_UP);
        }

        BigDecimal giaBanGoc = chiTietSanPham.getGiaBan() != null
                ? chiTietSanPham.getGiaBan()
                : BigDecimal.ZERO;

        if (chiTietSanPham.getChiTietDotGiamGias() == null
                || chiTietSanPham.getChiTietDotGiamGias().isEmpty()) {
            return giaBanGoc.setScale(2, java.math.RoundingMode.HALF_UP);
        }
        ...
        if (bestDiscount == null || bestDiscount.getIdDotGiamGia() == null) {
            return giaBanGoc.setScale(2, java.math.RoundingMode.HALF_UP);
        }

        BigDecimal discountPercent = BigDecimal
                .valueOf(bestDiscount.getIdDotGiamGia().getGiaTriGiamGia());
        ...
        return giaBanGoc.multiply(hundred.subtract(discountPercent))
    ...
    public Integer themSanPham(Integer idHoaDon, Integer idChiTietSanPham, Integer soLuong, Integer idNhanVien) {
        ...
        BigDecimal giaBanSauGiam = calculateGiaBanSauGiam(chiTietSanPham);
