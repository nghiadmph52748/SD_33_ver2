...
public class BanHangService {
	...
	public Object taoHoaDon(Integer idNhanVien) {
		...
		// Call stored procedure to generate invoice code (procedure will UPDATE
		// ma_hoa_don directly)
	...
	private String generateInvoiceCode(Integer idHoaDon) {
		try {
			// Call stored procedure with idHoaDon parameter - procedure will UPDATE the
			// column directly
	...
	public void xoaHoaDon(Integer idHoaDon, Integer idNhanVien) {
		HoaDon hd = hdRepository.findById(idHoaDon)
	...
	public Integer themSanPham(Integer idHoaDon, Integer idChiTietSanPham, Integer soLuong, Integer idNhanVien) {
		...
		// This ensures each add operation has its own line item that can be
		...
		if (chiTietSanPham.getChiTietDotGiamGias() != null && !chiTietSanPham.getChiTietDotGiamGias().isEmpty()) {
			ChiTietDotGiamGia[] ctdg = chiTietSanPham.getChiTietDotGiamGias().stream()
					.filter(ctdgItem -> ctdgItem != null && ctdgItem.getTrangThai() && !ctdgItem.getDeleted()
							&& ctdgItem.getIdDotGiamGia() != null
							&& ctdgItem.getIdDotGiamGia().getNgayBatDau() != null
							&& ctdgItem.getIdDotGiamGia().getNgayKetThuc() != null
							&& ctdgItem.getIdDotGiamGia().getNgayBatDau().isBefore(LocalDateTime.now())
							&& ctdgItem.getIdDotGiamGia().getNgayKetThuc().isAfter(LocalDateTime.now()))
					.toArray(ChiTietDotGiamGia[]::new);
			if (ctdg != null && ctdg.length > 0) {
				ChiTietDotGiamGia ctdg1 = new ChiTietDotGiamGia();
				if (ctdg.length > 1) {
					for (ChiTietDotGiamGia c : ctdg) {
						if (c.getIdDotGiamGia() != null && (ctdg1.getIdDotGiamGia() == null
								|| c.getIdDotGiamGia().getGiaTriGiamGia() > ctdg1.getIdDotGiamGia().getGiaTriGiamGia())) {
							ctdg1 = c;
						}
					}
				} else {
				...
				if (ctdg1.getIdDotGiamGia() != null) {
					BigDecimal giaTriGiam = new BigDecimal(ctdg1.getIdDotGiamGia().getGiaTriGiamGia());
		...
		hdct.setCreateAt(LocalDateTime.now());
	...
	public void xoaSanPham(Integer[] idChiTietSanPham, Integer idNhanVien) {
		...
		for (Integer idCtsp : idChiTietSanPham) {
			HoaDonChiTiet hdct = hdctRepository.findById(idCtsp)
					.orElseThrow(() -> new ApiException("Không tìm thấy chi tiết hóa đơn với id: " + idCtsp, "404"));

			ChiTietSanPham ctsp = ctspRepository.findById(hdct.getIdChiTietSanPham().getId()).orElseThrow();

			hdct.setDeleted(true);
			hdct.setUpdateAt(LocalDateTime.now());
	...
	public void updateSoLuongSanPham(Integer idHoaDonChiTiet, Integer soLuong, Integer idNhanVien) {
		...
		if (diff > 0) {
			...
			if (ctsp.getSoLuong() < diff) {
				throw new ApiException("Tồn kho không đủ! Yêu cầu tăng: " + diff + " | Còn lại: " + ctsp.getSoLuong(),
						"400");
			}
		}
		...
		ctspService.updateVariantStatusByQuantity(ctsp);
		// Cập nhật tổng tiền hóa đơn
		updateTongTienHoaDon(hdct.getIdHoaDon());
	...
	public Integer updateKhachHang(Integer idHoaDon, Integer idKhachHang, String tenKhachHang, String soDienThoai,
		...
		if (idKhachHang != null) {
			...
			KhachHang khachHang = khRepository.findById(idKhachHang)
					.orElseThrow(() -> new ApiException("Không tìm thấy khách hàng với id: " + idKhachHang, "404"));
	...
	public void banHangTaiQuay(ConfirmBanHangRequest request) {
		...
		BigDecimal tienChuyenKhoan = request.getTienChuyenKhoan() != null ? request.getTienChuyenKhoan()
				: BigDecimal.ZERO;
	...
	public void kiemTra(Map<Integer, Integer> listIdChiTietSanPham, Integer idPhieuGiamGia, Integer idNhanVien,
		...
		if (idPhieuGiamGia != null) {
			...
			if (!result) {
				throw new ApiException("Phiếu giảm giá với id: " + idPhieuGiamGia
						+ " không đủ tồn kho hoặc khách hàng không được phép sử dụng", "400");
			}
		}
		Boolean nvTT = nvRepository.findById(idNhanVien)
		...
		if (idKhachHang != null) {
			Boolean khTT = khRepository.findById(idKhachHang)
					.orElseThrow(() -> new ApiException("Không tìm thấy khách hàng với id: " + idKhachHang, "404"))
					.getTrangThai();
			if (!khTT) {
		...
		// Note: idPhuongThucThanhToan is actually idPTTT (1:cash, 2:transfer, 3:both) -
		// not a database ID
	...
	public void trangThaiDonHangOffline(Integer idHoaDon) {
		...
		thongTinDonHang2.setThoiGian(LocalDateTime.now());
		...
		thongTinDonHang4.setIdTrangThaiDonHang(getOrderStatus(7));
		thongTinDonHang4.setThoiGian(LocalDateTime.now());
