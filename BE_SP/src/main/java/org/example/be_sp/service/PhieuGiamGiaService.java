...
public class PhieuGiamGiaService {
    ...
    public List<PhieuGiamGiaResponse> getCouponsForCustomer(Integer idKhachHang) {
        ...
        // Get all public coupons (featured = false) with available quantity (soLuongDung > 0)
        List<PhieuGiamGia> publicCoupons = phieuGiamGiaRepository.findAll().stream()
                .filter(pgg -> !pgg.getFeatured() && pgg.getTrangThai() && pgg.getSoLuongDung() != null && pgg.getSoLuongDung() > 0)
                .toList();

        for (PhieuGiamGia pgg : publicCoupons) {
            PhieuGiamGiaResponse response = new PhieuGiamGiaResponse(pgg);
            Long usageCount = phieuGiamGiaRepository.countUsageByPhieuGiamGiaId(pgg.getId());
            response.setSoLuongDaDung(usageCount != null ? usageCount.intValue() : 0);
            result.add(response);
        ...
        // Get personal coupons for this customer (featured = true and trangThai = true with available quantity)
        ...
        for (PhieuGiamGiaCaNhan pggcn : personalCoupons) {
            PhieuGiamGia pgg = pggcn.getIdPhieuGiamGia();
            // Check: 
            // 1. PhieuGiamGiaCaNhan is NOT used (trangThai = true) - this voucher is still available for this customer
            // 2. Parent PGG is active (trangThai = true)
            // 3. Parent PGG has available quantity (soLuongDung > 0)
            if (pgg != null
                    && pggcn.getTrangThai() != null && pggcn.getTrangThai() // Personal voucher not used yet
                    && pgg.getTrangThai() // Parent voucher is active
                    && pgg.getSoLuongDung() != null && pgg.getSoLuongDung() > 0) // Has quantity available
            {
                PhieuGiamGiaResponse response = new PhieuGiamGiaResponse(pgg, false);  // personalUsed = false (not used yet)
                Long usageCount = phieuGiamGiaRepository.countUsageByPhieuGiamGiaId(pgg.getId());
                response.setSoLuongDaDung(usageCount != null ? usageCount.intValue() : 0);
                result.add(response);
            }
    ...
    /**
     * Safe equals for nullable objects Special handling for BigDecimal to use
